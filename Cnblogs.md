...
- 关于`Cnblogs`的主题样式介绍，最近在将文章搬运到`Cnblogs`，所以就稍微倒腾了一下主题。
- 本博客使用的主题为[`cnblogs-theme-silence`](https://esofar.github.io/cnblogs-theme-silence/#/)，并在此基础上做了一点点小更改。

# CSS样式

- 可以前去[`cnblogs-theme-silence`](https://esofar.github.io/cnblogs-theme-silence/#/)中找到相应的`silence.min.css`文件，并获取到相似的`CSS`样式代码。
- 以下为本人修改过一点点之后，`CSS`样式的代码：
...
* {
    margin: 0;
    padding: 0;
    cursor: url(https://files.cnblogs.com/files/phax2/cursor.ico), auto;
}
...
.cnblogs-markdown .hljs, .cnblogs-markdown .hljsln {
    ...
    font-size: 15px !important;
    /*  Customize Font */
    font-family: Cascadia Code, Consolas, Microsoft YaHei UI Light, Monaco, Andale Mono, Ubuntu Mono, monospace !important;
}
...
#main .postTitle2, a.entrylistItemTitle {
    font-weight: 700;
}
...
a#Header1_HeaderTitle {
    font-family: Consolas, Microsoft YaHei UI Light, Monaco, Andale Mono, Ubuntu Mono, monospace;
    font-weight: 300;
}
...
- 注意，需要先取得`Cnblogs`的`JS`权限后，才能进行此部分的配置。
- 同样，可以前去[`cnblogs-theme-silence`](https://esofar.github.io/cnblogs-theme-silence/#/)中找到相应文档，查看如何编写此部分的代码。
- 以下为额外添加了一点点小修改的`JS`代码：

```JavaScript
<script>
    window.$silence = {
        avatar: 'https://files.cnblogs.com/files/phax2/avatar.gif',
        favicon: 'https://files.cnblogs.com/files/phax2/favicon.ico',
        github: 'https://github.com/2phangx-dylan',
        defaultMode: 'light',
        defaultTheme: 'a',
        showNavAdmin: false,
        hljsln: true,
    ...
    $(function () {
        $("#profile_block").find("a").first().html(""); // 不显示用户名
        $("#blog_post_info_block").remove(); // 不显示点赞图标等
        $(".c_b_p_desc_readmore").remove(); // 首页不显示“阅读全文”提示
        $($("#profile_block").find("a")[1]).removeAttr("title").html("π"); // 调皮一下园龄
        $("#sidebar_search").find(".catListTitle").first().html("搜索一下"); // 修改搜索文本

        /* 不显示“摘要：” */
        var $descs = $(".c_b_p_desc");
        for (var i = $descs.length - 1; i >= 0; i--) {
            var afterText = $descs[i].innerHTML.replace("摘要：", "");
            $($descs[i]).html(afterText);
        }
    })
</script>
<script src="https://cdn.jsdelivr.net/gh/esofar/cnblogs-theme-silence@3.0.0-beta3/dist/silence.min.js"></script>
<script src="https://files.cnblogs.com/files/phax2/mouse-click.js"></script>
<canvas width="10000" height="10000" style="position: fixed; left: 0px; top: 0px; z-index: 2147483647; pointer-events: none;"></canvas>
```

- 需要留意其中`https://files.cnblogs.com/files/phax2/`开头的`url`，这些文件都是存在于本人的博客文件中的，你可以自行下载并放置到你的网盘或图床中，同样`Github`的`url`也请自行修改。

# HTML代码

- 页首代码同样来源于[`cnblogs-theme-silence`](https://esofar.github.io/cnblogs-theme-silence/#/)中的建议，用于加载缓冲。
- 代码如下：
...
- 以下直接贴一个完整的配置截图，只需要将以上代码复制粘贴到对应文本框，保存设置即可生效。
- 详情如下：

![image-20201230222410637](images/Cnblogs.images/image-20201230222410637.png)

# 总结

- 以上为`Cnblogs`主题样式的所有内容。